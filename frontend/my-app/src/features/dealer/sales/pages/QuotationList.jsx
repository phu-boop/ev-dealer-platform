...
import { getQuotationsByDealer, updateQuotationStatus, getMyQuotations } from '../services/salesService';
...
const QuotationList = () => {
    ...
    const fetchQuotations = async () => {
        ...
        try {
            ...
            setError("Không thể tải danh sách báo giá. " + (err.response?.data?.message || err.message));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleUpdateStatus = async (id, newStatus) => {
        ...
        try {
            ...
            Swal.fire('Thất bại!', `Lỗi: ${err.response?.data?.message || err.message}`, 'error');
        }
    };
    ...
    return (
        <div className="p-6 bg-white rounded-2xl shadow-lg">
            ...
            <div className="flex space-x-2 border-b border-gray-200 mb-4">
                {['PENDING', 'APPROVED', 'REJECTED', 'DRAFT', 'EXPIRED'].map((status) => (
                    <button
                        key={status}
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Khách hàng (ID)</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Xe (Variant ID)</th>
                        ...
                        {quotations.length === 0 ? (
                            ...
                            quotations.map((quote) => (
                                <tr key={quote.quotationId} className="hover:bg-gray-50">
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{formatDate(quote.quotationDate)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{quote.customerId}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{quote.variantId} (Model: {quote.modelId})</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 font-semibold">{formatCurrency(quote.finalPrice)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div className="flex space-x-2">
                                            ...
                                            {/* Nút Sửa */}
                                            {(
                                                (isManager && (quote.status === 'PENDING' || quote.status === 'DRAFT')) ||
                                                (!isManager && quote.status === 'DRAFT' && quote.staffId === memberId)
                                            ) && (
                                                <Button variant="primary" onClick={() => handleEdit(quote.quotationId)} title="Chỉnh sửa">
                                                    <FiEdit />
                                                </Button>
                                            )}
                                            ...
                                            {isManager && quote.status === 'PENDING' && (
                                                <>
                                                    <Button variant="success" onClick={() => handleUpdateStatus(quote.quotationId, 'APPROVED')} title="Duyệt">
                                                        <FiCheck />
                                                    </Button>
                                                    <Button variant="danger" onClick={() => handleUpdateStatus(quote.quotationId, 'REJECTED')} title="Từ chối">
                                                        <FiX />
                                                    </Button>
                                                </>
                                            )}
