...
//             onChange={(e) => handleChange('variantId', parseInt(e.target.value) || '')}
...
//             <p className="mt-1 text-sm text-red-600">{validationErrors.variantId}</p>
//           )}
//         </div>
...
//           <span className="text-sm font-medium text-blue-900">Thành tiền tạm tính:</span>
...
const OrderItemForm = ({ 
  ...
  useEffect(() => {
    const loadModelAndVariants = async () => {
      ...
      try {
        ...
        if (variantsResponse && variantsResponse.code === "1000" && variantsResponse.data) {
        ...
        if (promotionsResponse && Array.isArray(promotionsResponse)) {
          ...
          console.log('ℹ️ Không có promotions nào hoặc response không đúng định dạng');
        }
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      'IN_PRODUCTION': { color: 'bg-green-100 text-green-800', text: 'Đang sản xuất' },
      'COMING_SOON': { color: 'bg-blue-100 text-blue-800', text: 'Sắp ra mắt' },
      'DISCONTINUED': { color: 'bg-red-100 text-red-800', text: 'Ngừng sản xuất' }
    };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {loadingData && (
        <div className="flex items-center justify-center p-6 bg-blue-50 rounded-lg">
          ...
          <span className="ml-3 text-blue-700 font-medium">Đang tải thông tin xe và khuyến mãi...</span>
        </div>
      )}
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Chọn phiên bản xe</h3>
        ...
        {variants.length > 0 ? (
          <>
              ...
              >
                ...
                {variants.map((variant) => (
                  <option key={variant.variantId} value={variant.variantId}>
                    {variant.versionName} - {variant.color} - {formatCurrency(variant.price)}
                  </option>
                ))}
              </select>
            </div>
            ...
            {selectedVariant && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
                <div>
                  <h4 className="font-medium text-gray-900">{selectedVariant.versionName}</h4>
                  <p className="text-sm text-gray-600">Màu: {selectedVariant.color}</p>
                  <p className="text-sm text-gray-600">SKU: {selectedVariant.skuCode}</p>
                  <div className="mt-2">{getStatusBadge(selectedVariant.status)}</div>
                </div>
          ...
          !loadingData && (
            <div className="text-center py-4 text-gray-500">
              ...
              <p className="text-sm mt-1">Vui lòng kiểm tra lại thông tin đơn hàng.</p>
            </div>
          )
        )}
      </div>
      ...
      {promotions.length > 0 && (
        <div className="bg-yellow-50 rounded-lg border border-yellow-200 p-4">
          <h3 className="text-lg font-medium text-yellow-900 mb-4">Khuyến mãi hiện có</h3>
            ...
            >
              ...
              {promotions.map((promotion) => (
                <option key={promotion.promotionId} value={promotion.promotionId}>
                  {promotion.promotionName} - Giảm {(promotion.discountRate * 100)}%
                </option>
              ))}
            </select>
          </div>
          ...
          {promotions.map(promotion => (
            <div key={promotion.promotionId} className="p-3 bg-white rounded border border-yellow-100 mb-2">
              <div className="flex justify-between items-start">
                <div>
                  <h4 className="font-medium text-yellow-800">{promotion.promotionName}</h4>
                  <p className="text-sm text-yellow-600">{promotion.description}</p>
      ...
      <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200">
        ...
        <button
          type="submit"
          disabled={loading || !formData.variantId || finalPrice < 0 || variants.length === 0}
          className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium flex items-center gap-2"
        >
          {loading && (
            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
