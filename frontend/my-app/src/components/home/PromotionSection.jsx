...
// Tôi thay Button custom bằng thẻ button thường để đảm bảo không bị lỗi hiển thị text
// Nếu bạn muốn dùng Button component cũ, hãy chắc chắn nó nhận class text-gray-900
...
const PromotionSection = () => {
  ...
  return (
    // KỸ THUẬT QUAN TRỌNG:
    // w-screen: Chiếm toàn bộ chiều rộng màn hình
    // left-[calc(-50vw+50%)]: Căn chỉnh lại vị trí để tràn viền kể cả khi nằm trong container
          ...
          {promotions.map((promo) => (
            <div key={promo.id} className="group relative bg-gray-800/40 backdrop-blur-md rounded-3xl border border-gray-700 hover:border-gray-500 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-900/20">
              ...
              <div className="h-full p-8 flex flex-col relative overflow-hidden rounded-3xl">
                ...
                <div className={`absolute -top-20 -right-20 w-40 h-40 bg-gradient-to-br ${promo.color} opacity-0 blur-[60px] group-hover:opacity-30 transition-opacity duration-500`}></div>

                <div className="flex justify-between items-start mb-6 relative z-10">
                  <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-gradient-to-r ${promo.color} text-white shadow-lg`}>
                    {promo.badge}
                  </span>
                  <div className="flex items-center text-xs font-medium text-gray-400 bg-gray-900/50 px-3 py-1 rounded-full border border-gray-700">
                    <Clock className="w-3 h-3 mr-2" /> {promo.expiry}
                  </div>
                </div>
                ...
                <div className="mt-auto relative z-10">
                    {/* Sử dụng thẻ button thường để fix lỗi hiển thị text */}
