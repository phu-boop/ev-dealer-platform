...
export default function ProductionPlanPage() {
  ...
  const handleGenerate = async () => {
    try {
      setLoading(true);
      const response = await forecastService.generateProductionPlan(selectedMonth);
      setPlans(response.data || []);
      alert('Kế hoạch sản xuất đã được tạo thành công!');
    } catch (error) {
      console.error('Error generating plan:', error);
      alert('Lỗi khi tạo kế hoạch. Vui lòng thử lại.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const totalStats = plans.reduce((acc, plan) => {
    return {
      totalProduction: acc.totalProduction + (plan.recommendedProduction || 0),
      ...
      {plans.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{totalStats.totalProduction}</div>
              <p className="text-xs text-muted-foreground">đơn vị cần sản xuất</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-orange-200 bg-orange-50">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-orange-600">{totalStats.totalGap}</div>
          ...
          <Card className="border-red-200 bg-red-50">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-600">{totalStats.highPriority}</div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-3">
                          <h3 className="font-bold text-lg">{plan.variantName}</h3>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
                          <div>
                            <div className="text-xs text-muted-foreground">Đề Xuất SX</div>
                            <div className="text-lg font-bold text-blue-600">
                              {plan.recommendedProduction}
                            </div>
                          </div>
                          <div>
                            <div className="text-xs text-muted-foreground">Nhu Cầu DT</div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700 mt-1">
                  Chọn tháng và nhấn "Tạo Kế Hoạch" để AI tự động tạo kế hoạch sản xuất 
